...
export default function ImageCropper({ imageUrl, onCropComplete, onCancel, aspect: initialAspect = 3 / 4 }: ImageCropperProps) {
    const { t } = useLanguage();
    const imgRef = useRef<HTMLImageElement | null>(null);
    const [aspect] = useState<number | undefined>(initialAspect);
    ...
    const handleApply = async () => {
        ...
        try {
            ...
            const storageRef = ref(storage, `blocks/cropped_${timestamp}.jpg`);
                ...
                <div className="flex-1 min-h-0 relative flex items-center justify-center bg-[#050505] p-4 overflow-auto">
                    <div className="w-full h-full flex items-center justify-center">
                        ...
                        >
                            ...
                            />
                            {/* Visual Guides to match Library aesthetics */}
                            <div className="absolute inset-0 pointer-events-none border border-white/20">
                                {/* Read Now Bottom Gradient */}
                                <div className="absolute inset-x-0 bottom-0 h-1/4 bg-gradient-to-t from-blue-950/90 to-transparent flex items-end p-2">
                                    <span className="text-[8px] font-black tracking-wider text-blue-300 uppercase opacity-60">
                                        {t.readNow} →
                                    </span>
                                </div>

                                {/* Corner Accents */}
                                <div className="absolute top-0 left-0 w-3 h-3 border-t border-l border-blue-400 opacity-60" />
                                <div className="absolute bottom-0 right-0 w-3 h-3 border-b border-r border-blue-400 opacity-60" />
                            </div>
                        </ReactCrop>
                    </div>
                </div>
